...
- **Total Parameters**: 4,641 parameters (CPU-friendly, inference time $\approx 0.08$ ms).
...
### 6.2 Part 2: Fresh Holdout Comparison (B vs C1 vs C2 across 5 fresh seeds: 19026–19030)
...
For timesteps where substitution was active, offline evaluation against the ground truth $y_{\text{true}}$ yielded the following average errors (`results/metrics/aux_virtual_sensor_quality.csv`):
...
**Negligible (< 0.15%).**  
The compact auxiliary LSTM has only 4,641 parameters. On CPU, single-step auxiliary inference averages **0.08 ms** per step. Compared to the MPC SLSQP solver time (averaging 60.5 ms median / 68.0 ms mean per solve), the extra runtime overhead is less than **0.15%**, which has zero impact on real-time feasibility.
