...
def verify_dataset_and_model() -> dict[str, float]:
    ...
    assert [len(splits[name]) for name in splits] == [18, 6, 6]
    assert all(
        set(splits[first]).isdisjoint(splits[second])
        for first, second in (("train", "validation"), ("train", "test"), ("validation", "test"))
    )
    assert set(np.concatenate(list(splits.values()))) == set(data["run_ids"].astype(int))
    for name, run_ids in splits.items():
        assert set(np.unique(data[f"window_run_ids_{name}"])) == set(run_ids)
    ...
    for name, expected in expected_stats.items():
        assert np.allclose(expected, data[f"normalization_{name}"], atol=1e-10)
    assert np.allclose(data["X_train"].mean((0, 1), dtype=np.float64), 0.0, atol=1e-4)
    assert np.allclose(data["X_train"].std((0, 1), dtype=np.float64), 1.0, atol=1e-4)
    ...
    assert config["dataset"] == "data/processed/dc_motor_lstm_dataset.npz"
    ...
    assert np.allclose(recursive_rmse, saved["recursive_rmse_by_horizon"], atol=1e-6)
    return {
        **test,
...
def verify_controller_results() -> tuple[dict[str, float | int | bool], dict, dict]:
    ...
    assert config["evaluation_role"] == "untouched_final_holdout"
    assert config["seed_repetitions"] == sorted(expected_seeds)
    assert_close(config["plant_timestep_s"], 0.01)
    assert_close(config["control_interval_s"], 0.05)
    assert config["control_interval_steps"] == 5
    assert config["mpc"]["horizon"] == 20
    assert config["mpc"]["move_blocks"] == [5, 15]
    assert len(config["mpc"]["move_blocks"]) == 2
    assert config["mpc"]["control_interval_steps"] == 5
    assert config["mpc"]["max_iterations"] == 8
    assert config["selected_strategy"] == "fixed_multirate_blocks"
    assert summary["selected_configuration"] == config
    assert summary["statistical_repetitions"] == 5

    counts = Counter((row["controller"], row["scenario"]) for row in runs)
    assert len(runs) == 220 and len(comparison) == 44
    assert len(counts) == 44 and set(counts.values()) == {5}
    assert {row["controller"] for row in runs} == expected_controllers
    assert {row["scenario"] for row in runs} == expected_scenarios
    assert {int(row["seed"]) for row in runs} == expected_seeds
    ...
    assert required_columns <= runs[0].keys()
    ...
    for key, repetitions in counts.items():
        ...
        assert int(float(aggregate["repetitions"])) == repetitions
    ...
    assert safety == summary["safety"]
    assert set(safety.values()) == {0}

    assert len(runtime_samples) == 3 * 11 * 5 * 121
    assert {row["controller"] for row in runtime_samples} == expected_controllers - {"A_PI"}
    assert {row["scenario"] for row in runtime_samples} == expected_scenarios
    assert {int(row["seed"]) for row in runtime_samples} == expected_seeds
    runtime_keys = [
    ...
    assert len(samples_per_run) == 3 * 11 * 5 and set(samples_per_run.values()) == {121}
    compute_ms = np.asarray([float(row["compute_ms"]) for row in runtime_samples])
    assert np.isfinite(compute_ms).all() and (compute_ms >= 0).all()
    assert all(row["optimizer_success"].lower() == "true" for row in runtime_samples)
    for row in runtime_samples:
        sample_index = int(row["sample_index"])
        assert sample_index % 5 == 0
        assert int(row["control_step"]) == sample_index // 5
    ...
    assert summary["runtime"]["measurement"] == "MPC solver/control-computation time"
    ...
    assert bool(summary["runtime"]["real_time_20_hz"]) == expected_real_time

    assert len(virtual_quality) == 2 * 6 * 5
    assert {row["controller"] for row in virtual_quality} == {"C_sensor_MPC", "D_adaptive"}
    assert {row["scenario"] for row in virtual_quality} == {
    ...
    for row in virtual_quality:
        ...
        for field in ("virtual_feedback_RMSE", "corrupted_sensor_RMSE", "virtual_feedback_MAE", "corrupted_sensor_MAE"):
            ...
            assert (np.isfinite(value) and value >= 0) if duration else np.isnan(value)

    fallback = json.loads((METRICS / "pi_fallback_validation.json").read_text(encoding="utf-8"))
    assert fallback["forced_consecutive_updates"] == 10
    assert fallback["max_abs_speed_rad_s"] <= 120
    assert fallback["voltage_violations"] == fallback["slew_violations"] == 0
    assert fallback["mpc_resumed"]
    assert fallback["resume_voltage_jump_v"] <= fallback["resume_slew_limit_v"] + 1e-8

    with np.load(RAW / "final_representative_traces.npz") as traces:
        assert len(traces.files) > 0
        assert all(np.isfinite(traces[name]).all() for name in traces.files if name.endswith(("__true", "__voltage")))
...
def verify_recovery_logic() -> None:
    ...
    assert config["health_residual"] == "raw_y_measured - y_hat"
    assert "CUSUM score" in config["recovery_conditions"]
    assert config["cusum_reset"] == "after complete persistent recovery only"
    ...
    for _ in range(config["enter_count"]):
        monitor.update(3.0)
    assert monitor.active and monitor.positive > config["threshold"]
    evidence = monitor.positive
    for _ in range(config["exit_count"]):
        monitor.update(0.0)
    assert monitor.active and 0 < monitor.positive < evidence
    while monitor.active:
        monitor.update(0.0)
    assert monitor.positive == monitor.negative == 0

    notebook = (ROOT / "notebooks/06_final_evaluation.ipynb").read_text(encoding="utf-8")
    assert "sensor.update(raw_y_measured - y_hat)" in notebook
    assert "y_feedback = y_hat if reliability" in notebook
    assert "event_mask &= TIME < event_end" in notebook
    assert "2.0 <= instant < 4.0" in notebook

    regression = read_csv("recovery_regression_trace.csv")
    assert len(regression) == 4 * 300
    assert {row["scenario"] for row in regression} == {
    ...
    assert float(lookup[("pre_fix", "combined_fault_load")]["false_recovery_events"]) == 1
    assert float(lookup[("post_fix", "combined_fault_load")]["false_recovery_events"]) == 0
    assert_close(
    ...
    assert all((baseline / name).stat().st_size > 0 for name in (
...
def verify_documentation(summary: dict, config: dict) -> None:
    ...
    for document in documents[:2]:
        ...
        assert not [token for token in tokens if token not in text]
...
def main() -> None:
    ...
    assert all((PLOTS / name).stat().st_size > 0 for name in required_plots)
