# Final Independent Post-Fix Repository Audit (Superseded Baseline)

This report originally covered the earlier MPC timing correction. Its preserved original is `results/baseline_prefix/FINAL_REPO_AUDIT_POSTFIX.md`; the recovery-specific independent verdict is `BUGFIX_AUDIT.md`.
...
| Reliability-aware sensor-fault RMSE | `1.374104` | `1.374104` | PASS |
| Sensor-fault improvement | `81.881647%` | `81.881647%` | PASS |
| Plain MPC disturbance/combined RMSE | `1.932806` | `1.932806` | PASS |
| Adaptive disturbance/combined RMSE | `4.709976` | `4.709976` | PASS |
| Signed adaptive degradation `(adaptive-plain)/plain` | `143.685962%` | `143.685962%` | PASS |

The positive adaptive value means a 143.685962% aggregate worsening. Corrected latching prevents false recovery in the combined case, but sustained virtual feedback performs poorly under the concurrent plant disturbance.
...
|---|---:|---:|---|
| Mean | `67.963908 ms` | `67.963908 ms` | PASS |
| Median | `60.554900 ms` | `60.554900 ms` | PASS |
| p95 | `135.782580 ms` | `135.782580 ms` | PASS |
| p99 | `166.619316 ms` | `166.619316 ms` | PASS |
| Maximum | `666.868900 ms` | `666.868900 ms` | PASS |
...
- Drift receives no material benefit in the current isolated-fault comparison.
- The corrected combined case has no false recovery but materially worse virtual-feedback/tracking error under plant mismatch.
...
| 11 | Sensor-fault improvement | PASS | Fresh raw runs reproduce `7.584044 -> 1.374104`, or `81.881647%`. |
| 12 | Adaptive disturbance result | PASS | Fresh raw runs reproduce `1.932806 -> 4.709976`, signed change `143.685962%`. |
